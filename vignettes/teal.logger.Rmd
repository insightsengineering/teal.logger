...
When using `register_logger` the logging level of the new logger defaults to `logger::INFO`. You can customize this behaviour in 4 different ways:
...
When using `register_logger` the log layout of the new logger defaults to `[{level}] {format(time, \"%Y-%m-%d %H:%M:%OS4\")} pid:{pid} token:[{token}] {ans} {msg}`. You can customize this behaviour in 4 different ways:
1. Using the `logger` interface.
```r
library(teal.logger)
register_logger("my_namespace")
...
`teal.logger`, by default emits logs to `stdout`. If you want to change its behaviour, you have to use [`logger`'s API](https://daroczig.github.io/logger/articles/customize_logger.html#delivering-log-records-to-their-destination-1).
